...
class BoneSegmentationNode(Node):
    def __init__(self):
        ...
        plt.draw()
        plt.pause(0.01)  # Show empty plot
    ...
    def transform_to_3d(self, segmented_coords):
        ...
        try:
            ...
            # print(f"Original seg_3D shape: {seg_3D.shape}")  # Debugging print

            # Ensure seg_3D is 3xN (remove extra rows if needed)
            if seg_3D.shape[0] > 3:
                seg_3D = seg_3D[:3, :]  # Keep only the first 3 rows

            # print(f"Fixed seg_3D shape: {seg_3D.shape}")  # Debugging print
    ...
    def update_plot(self):
        """ Updates the Matplotlib 3D plot with reconstructed points. """
        if len(self.reconstruction_points['x']) > 0:
            self.ax.clear()
            
            # Plot the reconstructed points
            self.ax.scatter(self.reconstruction_points['x'],
            ...
            if self.motion_buff is not None:
                ...
                pos_x = self.motion_buff.translation.x * 1000  # Convert to mm
